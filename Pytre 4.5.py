...
Version 4.5:
    10/07/2023: Adding a new category: Compensated. Controls that are already covered by other SOC servcies.
                Removed the not feasible right now control. Included in No Intent
...
colorCompensated = "#931bf5" #Purple
...
def writeJason(tecnicas, status):
    ...
    for line in tecnicas:
        ...
        if status[it] == "Coverage in place":
            ...
            color = colorNoIntent
        elif status[it] == "Compensated CTI":
            color = colorCompensated
        elif status[it] == "Compensated EDR":
            color = colorCompensated
        elif status[it] == "Compensated NDR":
            color = colorCompensated
        else:
